...
5. Worker and verifier sidecars: record worker coverage metadata, raw report paths, invocation modes, parent validation, readiness state, tooling preflight, verifier findings, and manual QA outside the ledger under `verifier-reports/`, `qa-report.json`, and worker coverage reports.
6. Fixes and source gaps: fix every `BLOCKING` or `MAJOR` verifier finding that the course sources can support. Unsupported in-scope answers must become `Source gap`; old-paper topics outside current authority must become `Out of scope`; unreadable questions, pages, tables, or diagrams must become `Unreadable`.
7. HTML render: render the learner artifact from `answer-ledger.json` through the `past-year-script` design system. Keep it self-contained and exam-ready, with source gaps and unreadable limitations only where they affect studying; detailed source refs, verifier status, readiness status, worker evidence, and proof metadata stay in sidecar proof files.
8. QA: create `qa-report.json` recording `design_system_id: past-year-script`, design-system files loaded, render checks, hash-router checks, reveal-control checks, one-paper-view checks, question-count reconciliation, source-gap handling, verifier-lane status, source-pack-as-syllabus assumptions, and any remaining limitations.
